...
async function main() {
    const UUID = await getVariableValue('UUID', '6c8610f0-6f74-49b6-8fd9-6c35c40a518b'); // 为保证安全隐蔽，建议留空，可在Node.js界面下的环境变量添加处（Environment variables）,点击ADD VARIABLE，修改变量
    ...
    const DOMAIN = await getVariableValue('DOMAIN', 'devgev-248234716359.us-central1.run.app');// 为保证安全隐蔽，建议留空，可在Node.js界面下的环境变量添加处（Environment variables）,点击ADD VARIABLE，修改变量
